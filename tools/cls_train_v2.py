...
import models.vehicle as customized_models
    ...
    for batch_idx, (inputs, targets) in enumerate(train_loader):
        ...
        if (batch_idx + 1) % cfg.CLS.disp_iter == 0:
            print('Training: [{}/{}][{}/{}] | Best_Acc: {:4.2f}% | Time: {:.2f} | Data: {:.2f} | '
                  ...
                  .format(epoch + 1, cfg.CLS.epochs, batch_idx + 1, len(train_loader), BEST_ACC, batch_time.average(),
...
class CustomData(data.Dataset):
    def __init__(self, img_path, txt_path, data_transforms=None):
    ...
    def __getitem__(self, item):
        ...
        img = Image.open(cfg.CLS.data_root+img_name)
...
def main():
    ...
    if cfg.CLS.train_crop_type == 'center':
    	train_aug = [transforms.Resize(cfg.CLS.base_size), 
    ...
    val_aug = [transforms.Resize(cfg.CLS.base_size),
                transforms.CenterCrop(cfg.CLS.crop_size),
                transforms.ToTensor(),
                normalize,]
